...
namespace Terraria.GameContent.Skies
{
  public class AmbientSky : CustomSky
  {
    ...
    private class ButterfliesSkyEntity : AmbientSky.FadingSkyEntity
    {
      ...
      public override void UpdateVelocity(int frameCount) => this.Velocity = new Vector2((float) ((0.10000000149011612 + (double) Math.Abs(Main.WindForVisuals) * 0.05000000074505806) * (this.Effects == SpriteEffects.FlipHorizontally ? -1.0 : 1.0)), 0.0f);
    ...
    private class PegasusSkyEntity : AmbientSky.FadingSkyEntity
    {
      ...
      public override void UpdateVelocity(int frameCount) => this.Velocity = new Vector2((float) ((1.5 + (double) Math.Abs(Main.WindForVisuals) * 0.60000002384185791) * (this.Effects == SpriteEffects.FlipHorizontally ? -1.0 : 1.0)), 0.0f);
    ...
    private class PixiePosseSkyEntity : AmbientSky.FadingSkyEntity
    {
      ...
      public override void UpdateVelocity(int frameCount) => this.Velocity = new Vector2((float) ((0.11999999731779099 + (double) Math.Abs(Main.WindForVisuals) * 0.079999998211860657) * (this.Effects == SpriteEffects.FlipHorizontally ? -1.0 : 1.0)), 0.0f);
    ...
    private class AirshipSkyEntity : AmbientSky.FadingSkyEntity
    {
      ...
      public override void UpdateVelocity(int frameCount) => this.Velocity = new Vector2((float) ((6.0 + (double) Math.Abs(Main.WindForVisuals) * 1.6000000238418579) * (this.Effects == SpriteEffects.FlipHorizontally ? -1.0 : 1.0)), 0.0f);
    ...
    private class EOCSkyEntity : AmbientSky.FadingSkyEntity
    {
      ...
      private void ZigzagMove(int frameCount) => this.Velocity = new Vector2((float) (this._direction * 3), (float) Math.Cos((double) frameCount / 1200.0 * 6.2831854820251465) * this._waviness);
