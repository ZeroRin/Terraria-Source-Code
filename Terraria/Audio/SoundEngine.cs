...
namespace Terraria.Audio
{
  public static class SoundEngine
  {
    ...
    public static SoundEffectInstance PlaySound(LegacySoundStyle type, Vector2 position) => SoundEngine.PlaySound(type, (int) position.X, (int) position.Y);

    public static SoundEffectInstance PlaySound(LegacySoundStyle type, int x = -1, int y = -1) => type == null ? (SoundEffectInstance) null : SoundEngine.PlaySound(type.SoundId, x, y, type.Style, type.Volume, type.GetRandomPitch());
    ...
    public static SlotId PlayTrackedSound(SoundStyle style, Vector2 position) => !SoundEngine.IsAudioSupported ? SlotId.Invalid : SoundEngine._player.Play(style, position);

    public static SlotId PlayTrackedSound(SoundStyle style) => !SoundEngine.IsAudioSupported ? SlotId.Invalid : SoundEngine._player.Play(style);
